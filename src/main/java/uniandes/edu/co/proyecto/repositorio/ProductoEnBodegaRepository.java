...
public interface ProductoEnBodegaRepository extends JpaRepository<ProductoEnBodega, ProductoEnBodegaPK> {
    ...
    @Query(value = "SELECT CANTIDAD_EN_BODEGA FROM PRODUCTO_EN_BODEGA WHERE IDENTIFICADOR_PRODUCTO = :idProducto AND ID_BODEGA = :idBodega", nativeQuery = true)
    Integer findCantidadEnBodega(@Param("idProducto") Integer idProducto, @Param("idBodega") Integer idBodega);
    ...
    //NEW NEW NEW 
    //Hacer calculo del costo promedio con query SQL
    @Modifying
    @Transactional
